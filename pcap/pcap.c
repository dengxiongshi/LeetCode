...
//
    if ((fp = fopen("C:\\Users\\Administrator\\Desktop\\资料\\n4-155.pcap", "r")) == NULL)
    {
        printf("error: can not open pcap file\n");
        exit(0);
    }
    if ((output = fopen("C:\\Users\\Administrator\\Desktop\\资料\\output.txt", "w +")) == NULL)
    {
...
//开始读数据包
    ...
    while (fseek(fp, pkt_offset, SEEK_SET) == 0) //遍历数据包
    {
        ...
        strftime(my_time, sizeof(my_time), "%Y -%m -%d % T", localtime((const time_t *) &(ptk_header->ts.tv_sec))); //获取时间
        // printf(“%d: %s\n”, i, my_time);
        //数据帧头 14字节
